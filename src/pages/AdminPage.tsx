...
const AdminPage = () => {
  ...
  const menuItems = [
    ...
    { id: "operations", label: "Operations", icon: Activity, color: "text-slate-500", bg: "bg-slate-50" },
  ...
  const MenuCard = ({ item }: { item: UnifiedItem }) => (
    <motion.div
      layout
      whileHover={{ y: -8 }}
      className={`group bg-white rounded-[2.5rem] border overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 ${item.tag === 'custom' ? 'border-purple-100 shadow-purple-50' : 'border-slate-100'
        }`}
    >
      <div className="h-56 relative overflow-hidden">
        ...
        <div className="absolute top-5 right-5 flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
          <button onClick={() => { setEditItem(item); setShowForm(true); }} className="p-3 bg-white/90 backdrop-blur-md rounded-2xl text-slate-800 shadow-xl hover:text-primary transition-colors"><Pencil size={20} /></button>
          <button onClick={() => handleDelete(item.id)} className="p-3 bg-white/90 backdrop-blur-md rounded-2xl text-destructive shadow-xl hover:bg-destructive hover:text-white transition-colors"><Trash2 size={20} /></button>
        </div>
        <div className="absolute bottom-5 left-5">
          <Badge className="bg-white/90 backdrop-blur-md text-slate-900 border-none font-black text-lg px-4 py-1.5 rounded-2xl shadow-xl">
            ${(item.price || 0).toFixed(2)}
          </Badge>
        </div>
        {item.tag === 'custom' && (
          <div className="absolute top-5 left-5">
            <Badge className="bg-purple-600 text-white border-none font-black text-[10px] uppercase px-3 py-1 rounded-lg shadow-xl tracking-widest">
              Custom Creation
            </Badge>
          </div>
        )}
      ...
      <div className="p-8">
        <div className="flex justify-between items-start mb-2">
          <h4 className="font-black text-2xl text-slate-900 tracking-tight leading-tight">{item.name}</h4>
        </div>
        <div className="flex items-center gap-3 mb-4">
          <Badge variant="secondary" className="bg-slate-100 text-slate-500 font-bold px-3 py-1 rounded-lg border-none uppercase text-[10px]">
            {item.category}
          </Badge>
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-body">
      {/* Sidebar Desktop */}
      <aside className="hidden lg:flex flex-col w-64 xl:w-72 bg-white border-r border-slate-200 shadow-sm z-30 transition-all duration-500">
        <div className="p-8 h-full flex flex-col">
          <div className="flex items-center gap-3 mb-10 group cursor-pointer" onClick={() => setTab("analytics")}>
            <div className="p-2.5 bg-slate-900 rounded-2xl shadow-xl shadow-slate-200 group-hover:scale-110 transition-transform">
              <Utensils className="h-6 w-6 text-white" />
            </div>
            <div>
              <h2 className="font-display text-2xl font-black text-slate-900 tracking-tighter leading-none">Admin<span className="text-primary italic">Pro</span></h2>
              <p className="text-[9px] uppercase font-black text-slate-400 tracking-[0.2em] mt-1">Management Suite</p>
            </div>
          </div>
            ...
            >
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-display font-black text-2xl text-slate-900 tracking-tight">AdminPro</span>
                </div>
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-black transition-all ${isActive ? "bg-slate-900 text-white shadow-xl shadow-slate-200" : "text-slate-500 hover:bg-slate-50"
                        }`}
                    >
              ...
              <div className="mt-8 pt-8 border-t border-slate-100">
                ...
                <Button variant="outline" className="w-full h-14 justify-start rounded-2xl font-black text-destructive border-slate-200" onClick={logout}>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto bg-slate-50/50">
        {/* Top Navbar */}
        <header className="sticky top-0 w-full bg-white/80 backdrop-blur-xl border-b border-slate-200 z-40 px-[5%] py-4 flex items-center justify-between">
          {/* Left Section */}
          <div className="flex-1 flex items-center">
            <Button variant="ghost" size="icon" className="lg:hidden rounded-xl bg-slate-50" onClick={() => setIsSidebarOpen(true)}>
          ...
          <div className="flex-1 flex justify-center items-center text-center">
            <div className="flex flex-col items-center">
              ...
              <h2 className="text-xs sm:text-lg font-black text-slate-900 tracking-tight uppercase sm:capitalize whitespace-nowrap">
          ...
          <div className="flex-1 flex items-center justify-end gap-[4%]">
            {(tab === "regular-items" || tab === "custom-items") && (
              <Button
                ...
                className="h-10 w-10 sm:h-11 sm:w-auto sm:px-6 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-black text-sm shadow-lg shadow-slate-200 transition-all hover:scale-105 flex items-center justify-center"
              >
            ...
            <div className="relative" ref={userMenuRef}>
              <button
                ...
                className="flex items-center gap-3 p-1.5 pr-3 rounded-2xl bg-white border border-slate-200 hover:border-primary/30 transition-all active:scale-95 shadow-sm"
              >
                <div className="w-9 h-9 rounded-[0.75rem] bg-gradient-to-br from-slate-900 to-slate-700 flex items-center justify-center text-white font-black text-xs">
                  {user.name?.[0] || "A"}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-black text-slate-900 leading-none">{user.name}</p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-1">Administrator</p>
                </div>
              ...
              <AnimatePresence>
                {isUserMenuOpen && (
                  <motion.div
                    ...
                    className="absolute top-full right-0 mt-3 w-[260px] sm:w-72 bg-white/95 backdrop-blur-xl rounded-[2.25rem] shadow-2xl border border-slate-100 z-50 overflow-hidden"
                  >
                    <div className="p-5 sm:p-6 bg-slate-50/50 border-b border-slate-100">
                      <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 whitespace-nowrap">Authenticated As</p>
                      <p className="font-black text-slate-900 truncate text-xs sm:text-sm">{user.email}</p>
                    </div>
                    <div className="p-2 sm:p-3">
                      <button
                        onClick={() => { setTab('profile'); setIsUserMenuOpen(false); }}
                        className="w-full flex items-center justify-between p-3.5 sm:p-4 rounded-2xl text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-bold transition-all group"
                      >
                        <div className="flex items-center gap-3">
                      ...
                      <div className="h-[1px] bg-slate-100 my-1.5 sm:my-2 mx-2" />
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-10 lg:p-16 pt-10">
            ...
            <div className="space-y-8 sm:space-y-12">
              ...
              {regularItems.length === 0 && (
                <div className="py-32 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
                  ...
                  <h3 className="text-2xl font-black text-slate-400">Empty Regular Vault</h3>
                  <p className="text-slate-400 font-medium mt-2">Initialize your offerings by creating your first regular menu item.</p>
          ...
          {!menuItems.some(i => i.id === tab) && (
            <div className="py-20 text-center">
              <p className="text-slate-400 font-bold">Please select a section from the sidebar.</p>
            </div>
          )}
        </div>
      </main>
...
const ItemForm = ({ item, onSave, onCancel, defaultTag }: any) => {
  ...
  return (
    <Card className="rounded-[3rem] border-none bg-white p-6 sm:p-10 md:p-16 shadow-2xl overflow-hidden relative">
      ...
      <div className="mb-10 flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          <h3 className="font-display text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{item ? "Evolve" : "Forge"} Creation</h3>
          <p className="text-slate-400 font-bold mt-2 uppercase text-[10px] tracking-[0.2em]">Culinary Specification Portal</p>
        </div>
        <div className="flex flex-col items-start sm:items-end gap-2">
          <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Creation Classification</Label>
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button
              onClick={() => setForm({ ...form, tag: 'regular' })}
              className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${form.tag === 'regular' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Regular
            </button>
            <button
              onClick={() => setForm({ ...form, tag: 'custom' })}
              className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${form.tag === 'custom' ? 'bg-purple-600 text-white shadow-lg shadow-purple-100' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Custom
            </button>
          </div>
      ...
      <div className="grid gap-6 md:gap-8">
        ...
        <div className="space-y-4 bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
          <div className="flex items-center gap-2 mb-2">
            ...
            <Label className="text-sm font-black text-slate-800">Visual Identity (Image Address)</Label>
          </div>

          <div className="flex flex-col sm:flex-row gap-6 items-center">
            <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl bg-white flex-shrink-0">
              <img
            ...
            <div className="flex-1 w-full space-y-3">
              <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Public URL Address</Label>
              <input
                value={form.image}
                onChange={e => setForm({ ...form, image: e.target.value })}
                placeholder="https://images.unsplash.com/photo-..."
                className="w-full h-14 rounded-2xl bg-white border-2 border-transparent focus:border-primary/20 transition-all px-6 font-bold text-slate-600 text-sm outline-none"
              />
              <p className="text-[10px] text-slate-400 font-medium italic">Provide a secure URL (HTTPS) for the best result.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="space-y-3">
            <Label className="text-xs font-black text-slate-800 ml-1 flex items-center gap-2">
            ...
            <input
              ...
              className="w-full h-16 rounded-[1.25rem] bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white transition-all px-6 font-bold text-slate-900 outline-none"
            />
          </div>
          <div className="space-y-3">
            <Label className="text-xs font-black text-slate-800 ml-1 flex items-center gap-2">
          ...
          <div className="space-y-3 md:col-span-2">
            <Label className="text-xs font-black text-slate-800 ml-1">Narrative Description</Label>
            <textarea
              rows={3}
              value={form.description}
              onChange={e => setForm({ ...form, description: e.target.value })}
              placeholder="Describe the essence of this creation..."
              className="w-full rounded-[1.5rem] bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white transition-all p-6 font-bold text-slate-900 outline-none resize-none"
            />
          </div>
          <div className="space-y-3">
            <Label className="text-xs font-black text-slate-800 ml-1">Classification</Label>
            <select
              value={form.category}
              onChange={e => setForm({ ...form, category: e.target.value })}
              className="w-full h-16 rounded-[1.25rem] bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white transition-all px-6 font-bold text-slate-900 outline-none appearance-none"
            >
          ...
          <div className="flex items-center gap-4 bg-slate-50 p-5 rounded-[1.25rem] border-2 border-transparent hover:border-emerald-100 transition-all">
            <input
              type="checkbox"
              id="isVeg"
              checked={form.isVeg}
            ...
            />
            <Label htmlFor="isVeg" className="font-black text-slate-700 cursor-pointer text-sm">Vegetarian Protocol</Label>
          </div>

          <div className="space-y-3">
            <Label className="text-xs font-black text-slate-800 ml-1">Initial Stock Level</Label>
            <input
              type="number"
              value={form.stock}
          ...
          <div className="space-y-3">
            <Label className="text-xs font-black text-slate-800 ml-1">Unit Type</Label>
            <select
              value={form.unit}
      ...
      <div className="mt-12 flex flex-col sm:flex-row gap-4">
        <Button
          onClick={() => onSave(form)}
          className="flex-1 h-20 rounded-[1.5rem] bg-slate-900 hover:bg-slate-800 text-white font-black text-xl shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98]"
        >
        ...
        <Button
          onClick={onCancel}
          variant="ghost"
          className="flex-1 h-20 rounded-[1.5rem] font-bold text-slate-500 hover:bg-slate-50"
        >
          Discard
        </Button>
      </div>
